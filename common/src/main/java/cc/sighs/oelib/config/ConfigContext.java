 ...
 *
 * <p>An {@code ConfigContext} is bound as a {@link ScopedValue} while a
 * {@link ConfigSchema} or {@link ConfigRecordCodecBuilder} call tree is
 * executing. It carries the current configuration identifier, the current
...
public final class ConfigContext {
     ...
     */
    public static <T> T withRoot(Identifier configId, List<ConfigValueMeta> fields, Supplier<T> supplier) {
     ...
     */
    public static <T> T withRoot(Identifier configId, @Nullable Class<?> rootRecordClass, List<ConfigValueMeta> fields, Supplier<T> supplier) {
        Objects.requireNonNull(configId);
        Objects.requireNonNull(fields);
        Objects.requireNonNull(supplier);
        var frame = new Frame(configId, rootRecordClass, fields, "", 0);
        return ScopedValue.where(CURRENT, frame).call(supplier::get);
    }
    ...
    /**
     * Returns the identifier of the configuration currently being defined,
     * or {@code null} if no context is active.
     *
     * @return the current configuration identifier, or {@code null}
     */
    @Nullable
    public static Identifier currentConfigId() {
    ...
    private record Frame(Identifier configId, @Nullable Class<?> recordClass, List<ConfigValueMeta> fields,
