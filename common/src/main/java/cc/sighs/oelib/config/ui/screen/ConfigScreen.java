...
public class ConfigScreen extends Screen {
    ...
    public ConfigScreen(Screen parent, String modid) {
        ...
        if (chosen == null) {
            this.configId = Identifier.fromNamespaceAndPath(modid, "none");
            this.unit = null;
            this.codec = null;
            this.fields = List.of();
    ...
    protected void init() {
        ...
        Set<Identifier> modsConfigsSet = new HashSet<>();
    ...
    public void extractRenderState(GuiGraphicsExtractor gui, int mouseX, int mouseY, float partialTick) {
        ...
        extractTransparentBackground(gui);
        int sidebar = sidebarWidth();
        int contentLeft = sidebar;
        int sliderX = sidebar - 14;
        gui.centeredText(this.font, this.title, this.width / 2, 12, -1);
        int left = sidebar + 16;
        if (searchBox != null) {
            searchBox.setX(left);
        ...
        gui.text(this.font, arrow, sliderX + 7 - this.font.width(arrow) / 2, arrowY, 0xFFFFFFFF);
        if (sidebarExpanded) {
            ...
            for (FieldRef ref : references) {
                if (refY + itemHeight >= refAreaTop && refY <= refAreaBottom) {
                    ...
                    gui.text(this.font, ref.label().getVisualOrderText(), refX + 2, refY, color);
                }
                refY += itemHeight;
            }
        ...
        if (saveButton != null) {
            saveButton.extractRenderState(gui, mouseX, mouseY, partialTick);
        }
        if (cancelButton != null) {
            cancelButton.extractRenderState(gui, mouseX, mouseY, partialTick);
        }

        if (!errors.isEmpty()) {
            for (Map.Entry<String, Component> entry : errors.entrySet()) {
                gui.text(this.font, entry.getValue(), contentLeft + 16, this.height - 40, 0xFFFF4040);
            }
        }
        if (lastTooltip != null && mouseXLast >= 0) {
            gui.setTooltipForNextFrame(this.font, lastTooltip, mouseXLast, mouseYLast);
        }
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amountX, double amountY) {
        int sidebar = sidebarWidth();
        int sliderX = sidebar - 14;
        if (mouseX >= 0 && mouseX <= sliderX && mouseY >= 0 && mouseY <= this.height && amountY != 0.0) {
            int itemHeight = this.font.lineHeight + 3;
            refScroller.offset(-amountY * itemHeight);
            return true;
        }
        if (listWidget != null && mouseX >= sliderX && mouseX <= this.width && mouseY >= contentTop && mouseY <= this.height - 32 && amountY != 0.0) {
            listWidget.offset(-amountY * 28.0);
            return true;
        }
