...
public final class FunctionUsageAnalyzer {
    ...
    public static <T> Set<String> analyzeUsedFunctions(Map<Identifier, T> dataPackData,
                                                       DataExpressionExtractor<T> dataExtractor) {
        Set<String> usedFunctions = new HashSet<>();
        for (Map.Entry<Identifier, T> entry : dataPackData.entrySet()) {
    ...
    private static <T> boolean shouldLoadDataPackFunctions(T data, Identifier location,
                                                           DataExpressionExtractor<T> dataExtractor) {
