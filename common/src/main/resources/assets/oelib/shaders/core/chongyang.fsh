...
void main() {
    ...
    float aspect = UboResolution.x / max(1.0, UboResolution.y);
    vec2 fragCoord = uv * UboResolution.xy;
    vec2 p = centered;
    p.x *= aspect;

    vec3 baseColor = mountainScene(fragCoord, UboResolution.xy, UboTime.x);
    ...
    float cloud = cloudNoise(uv * 2.0, UboTime.x * 0.3);
    ...
    for (float i = 0.0; i < 22.0; i++) {
        ...
        float t = UboTime.x * (0.4 + seed * 0.6) + seed * 6.28;
        ...
        particles += glow * fade * (0.6 + 0.4 * sin(UboTime.x * 4.0 + seed));
    }
    color += goldColor * particles * 0.7;

    float border = smoothstep(0.98, 0.8, length(centered * vec2(0.5, 0.9)));
    float borderSparkle = pow(cloudNoise(uv * 5.0, UboTime.x), 2.0);
    ...
    float finalAlpha = UboToastAlpha.x * softMask * 0.9;
