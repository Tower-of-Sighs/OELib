...
public final class ClientRegistrationHandler {
    ...
    public void perform(RegistrationAction action) {
        ...
        if (action instanceof KeyMappingAction kma) {
            var mapping = kma.mapping();
        ...
        if (action instanceof ColorItemAction colorItemAction) {
            var items = colorItemAction.items();
            var arr = new ItemLike[items.length];
            for (int i = 0; i < items.length; i++) arr[i] = Objects.requireNonNull(items[i].get());
            ColorProviderRegistry.ITEM.register(colorItemAction.color(), arr);
            return;
        }

        if (action instanceof ColorBlockAction colorBlockAction) {
            var blocks = colorBlockAction.blocks();
            var arr = new Block[blocks.length];
            for (int i = 0; i < blocks.length; i++) arr[i] = Objects.requireNonNull(blocks[i].get());
            ColorProviderRegistry.BLOCK.register(colorBlockAction.color(), arr);
            return;
        }

        if (action instanceof RenderTypeBlocksAction renderTypeBlocksAction) {
            var blocks = renderTypeBlocksAction.blocks();
            var arr = new Block[blocks.length];
            for (int i = 0; i < blocks.length; i++) arr[i] = Objects.requireNonNull(blocks[i].get());
            BlockRenderLayerMap.INSTANCE.putBlocks(renderTypeBlocksAction.type(), arr);
            return;
        }

        if (action instanceof RenderTypeFluidsAction renderTypeFluidsAction) {
            var fluids = renderTypeFluidsAction.fluids();
            var arr = new Fluid[fluids.length];
            for (int i = 0; i < fluids.length; i++) arr[i] = Objects.requireNonNull(fluids[i].get());
            BlockRenderLayerMap.INSTANCE.putFluids(renderTypeFluidsAction.type(), arr);
            return;
        }
        ...
        if (action instanceof EntityModelLayerAction entityModelLayerAction) {
            EntityModelLayerRegistry.registerModelLayer(
                    entityModelLayerAction.location(),
                    () -> entityModelLayerAction.definition().get()
            );
            return;
        }
