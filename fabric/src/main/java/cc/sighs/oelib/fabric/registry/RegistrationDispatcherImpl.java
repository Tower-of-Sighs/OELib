...
public final class RegistrationDispatcherImpl implements IRegistrationDispatcher {
    ...
    public void perform(RegistrationAction action) {
        ...
        if (action instanceof CreativeTabAppendStackAction appendAction) {
            ResourceKey<CreativeModeTab> tabKey = appendAction.tab();
        ...
        if (action instanceof CreativeTabModifyAction modifyAction) {
            ResourceKey<CreativeModeTab> tabKey = modifyAction.tab();
        ...
        if (action instanceof KeyMappingAction) {
            performKeyMapping((KeyMappingAction) action);
            return;
        }

        if (action instanceof ClientTooltipComponentAction<?>) {
            @SuppressWarnings("unchecked")
            ClientTooltipComponentAction<TooltipComponent> tooltipRaw =
                    (ClientTooltipComponentAction<TooltipComponent>) action;
            performClientTooltip(tooltipRaw);
            return;
        }

        if (FabricLoader.getInstance().getEnvironmentType() == EnvType.CLIENT) {
            if (action instanceof ColorItemAction colorAction) {
                ItemColor color = colorAction.color();
                Supplier<? extends ItemLike>[] items = colorAction.items();

                var arr = new ItemLike[items.length];
                for (int i = 0; i < items.length; i++) {
                    arr[i] = Objects.requireNonNull(items[i].get());
                }
                ColorProviderRegistry.ITEM.register(color, arr);
                return;
            }

            if (action instanceof ColorBlockAction blockAction) {
                BlockColor color = blockAction.color();
                Supplier<? extends Block>[] blocks = blockAction.blocks();

                var arr = new Block[blocks.length];
                for (int i = 0; i < blocks.length; i++) {
                    arr[i] = Objects.requireNonNull(blocks[i].get());
                }
                ColorProviderRegistry.BLOCK.register(color, arr);
                return;
            }

            if (action instanceof RenderTypeBlocksAction rtAction) {
                RenderType type = rtAction.type();
                Supplier<? extends Block>[] blocks = rtAction.blocks();
            ...
            if (action instanceof RenderTypeFluidsAction rfAction) {
                RenderType type = rfAction.type();
                Supplier<? extends Fluid>[] fluids = rfAction.fluids();

                var arr = new Fluid[fluids.length];
                for (int i = 0; i < fluids.length; i++) {
                    arr[i] = Objects.requireNonNull(fluids[i].get());
                }
            ...
            if (action instanceof EntityRendererAction<?>) {
                EntityRendererAction<Entity> era = (EntityRendererAction<Entity>) action;
                EntityRendererRegistry.register(era.type().get(), era.provider());
                return;
            }

            if (action instanceof EntityModelLayerAction emla) {
                ModelLayerLocation location = emla.location();
                Supplier<LayerDefinition> definition = emla.definition();
                EntityModelLayerRegistry.registerModelLayer(location, definition::get);
                return;
            }

            if (action instanceof ParticleProviderAction<?>) {
                ParticleProviderAction<ParticleOptions> ppa = (ParticleProviderAction<ParticleOptions>) action;
                ParticleFactoryRegistry.getInstance().register(ppa.type(), ppa.provider());
                return;
            }

            if (action instanceof EntityAttributeAction attrAction) {
                Supplier<? extends EntityType<? extends LivingEntity>> type = attrAction.type();
                Supplier<AttributeSupplier.Builder> attributes = attrAction.attributes();
                FabricDefaultAttributeRegistry.register(type.get(), attributes.get());
                return;
            }

            if (action instanceof SpawnPlacementAction<?>) {
                SpawnPlacementAction<Mob> spa = (SpawnPlacementAction<Mob>) action;
            ...
            if (action instanceof TradeWandererAction wandererAction) {
                boolean rare = wandererAction.rare();
                VillagerTrades.ItemListing[] trades = wandererAction.trades();
