...
public final class ConfigManager {
    ...
    public static void registerUnit(ConfigUnit<?> unit) {
        ...
        if (meta.fileName() == null || meta.fileName().isBlank()) {
            OELibConfig.LOGGER.error("Config {} has empty filename; please set meta.fileName()", meta.id());
        }
        if (meta.format() == null) {
            OELibConfig.LOGGER.error("Config {} has null format; please set meta.format()", meta.id());
        }
        if (meta.side() == null) {
            OELibConfig.LOGGER.error("Config {} has null side; please set meta.side()", meta.id());
        }
        OELibConfig.LOGGER.info("Registered config {} from mod {} side {} format {} filename {} directory {}",
    ...
    /**
     * Looks up a configuration unit by its id, searching the server
