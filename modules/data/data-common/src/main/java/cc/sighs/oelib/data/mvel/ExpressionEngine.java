...
public final class ExpressionEngine {
    ...
    public static void initialize(Set<String> requiredFunctions) {
        ...
        ExpressionFunctionRegistry registry = (name, ownerClass, methodName, parameterTypes) -> {
            ...
            try {
                ...
                OELibData.LOGGER.debug("Registered function: {} ({}#{})", name,
                        ownerClass.getSimpleName(), methodName);
            } catch (Exception e) {
                OELibData.LOGGER.warn("Failed to register function {} ({}#{}): {}",
        ...
        if (requiredFunctions != null && functionMap.isEmpty()) {
            OELibData.LOGGER.warn("No functions registered for required set {}. Falling back to full registration.", requiredFunctions);
            initialize(null);
            return;
        }

        initialized = true;
        OELibData.LOGGER.info("Expression engine initialized via generated registrars: {} registrars, {} functions",
    ...
    public static Object evaluate(String expression, Map<String, Object> context, boolean logErrors) {
        try {
            ...
            if (logErrors) {
                OELibData.LOGGER.error("Failed to evaluate expression: {}", expression, e);
            }
            throw e;
        }
    }
    ...
    public static void hotReload() {
        clear();
        initialize();
        OELibData.LOGGER.debug("Expression engine hot reload completed");
    }
        ...
        if (vars != null) {
            for (Map.Entry<String, String> var : vars.entrySet()) {
                try {
                    ...
                    OELibData.LOGGER.debug("Failed to evaluate variable {}: {}", var.getKey(), e.getMessage());
    ...
    public static void executeActions(List<String> actions, Map<String, Object> context) {
        ...
        for (String action : actions) {
            try {
                ...
                OELibData.LOGGER.error("Failed to execute action: {}", action, e);
            }
        }
    }
    ...
    public static boolean checkModLoadedCondition(Map<String, String> vars) {
        ...
        try {
            ...
            OELibData.LOGGER.debug("Failed to evaluate modLoaded condition: {}", e.getMessage());
            return true;
        }
    }
}