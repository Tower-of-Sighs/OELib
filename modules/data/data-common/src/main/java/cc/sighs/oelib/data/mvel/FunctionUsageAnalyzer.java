...
public final class FunctionUsageAnalyzer {
    ...
    public static <T> Set<String> analyzeUsedFunctions(Map<ResourceLocation, T> dataPackData,
        ...
        OELibData.LOGGER.debug("Found {} used functions in data packages: {}", usedFunctions.size(), usedFunctions);
    ...
    private static <T> boolean shouldLoadDataPackFunctions(T data, ResourceLocation location,
        ...
        try {
            ...
            if (modId != null) {
                ...
                OELibData.LOGGER.debug("Mod '{}' loaded status: {} for datapack {}", modId, isLoaded, location);
                return isLoaded;
            }
        } catch (Exception e) {
            OELibData.LOGGER.debug("Failed to evaluate modLoaded expression for {}: {}", location, e.getMessage());
        }
        return true;
    }
