...
public class DataManager<T> implements SimpleResourceReloadListener<Map<ResourceLocation, JsonElement>> {
    ...
    public ResourceLocation getFabricId() {
        return new ResourceLocation(OELibData.MOD_ID, "data_manager_" + dataClass.getSimpleName().toLowerCase());
    }
                    ...
                    if (annotation.supportArray() && json.isJsonArray()) {
                        ...
                        for (int i = 0; i < jsonArray.size(); i++) {
                            ...
                            if (result.result().isPresent()) {
                                ...
                                var vr = v.validate(dataObj, elementLocation);
                                if (vr.valid()) {
                                    if (vr.deferrable()) {
                        ...
                        if (result.result().isPresent()) {
                            ...
                            var v = getValidatorForNamespace(location.getNamespace());
                            var vr = v.validate(dataObj, location);
                            if (vr.valid()) {
                                if (vr.deferrable()) {
