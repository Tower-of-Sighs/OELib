...
public final class ExampleRegistry {
    private static final DeferredRegister<Item> ITEMS = DeferredRegister.create(Registries.ITEM, OELibDev.MOD_ID);
    public static final RegisterSupplier<Item> EXAMPLE_ITEM = ITEMS.register("example_item", () -> new ExampleItem(new Item.Properties()));
    private static final DeferredRegister<Block> BLOCKS = DeferredRegister.create(Registries.BLOCK, OELibDev.MOD_ID);
    ...
    public static void init() {
        BLOCKS.register();
        ITEMS.register();
        OELibDev.LOGGER.info("Queued example registration: {}", EXAMPLE_ITEM.id() + " " + EXAMPLE_BLOCK.id());
    }
