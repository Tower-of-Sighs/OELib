...
loom {
    val aw = project(":modules:dev:dev-common").file("src/main/resources/oelib-dev.accesswidener")
    if (aw.exists()) {
    ...
    runs {
        named("client") {
            client()
            configName = "Fabric Client - OELibDev"
            ideConfigGenerated(true)
            runDir("run")
        }
        named("server") {
            server()
            configName = "Fabric Server - OELibDev"
            ideConfigGenerated(true)
            runDir("run")
        }
    }
