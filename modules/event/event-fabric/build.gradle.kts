...
loom {
    val aw = project(":modules:event:event-common").file("src/main/resources/oelib-event.accesswidener")
    if (aw.exists()) {
    ...
    runs {
        named("client") {
            client()
            configName = "Fabric Client - OELibEvent"
            ideConfigGenerated(true)
            runDir("run")
        }
        named("server") {
            server()
            configName = "Fabric Server - OELibEvent"
            ideConfigGenerated(true)
            runDir("run")
        }
    }
