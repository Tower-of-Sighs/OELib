package cc.sighs.oelib.misc.forge.scan;
...
public final class ForgeScanDataProvider implements IScanDataProvider {
    private final VTask<OELScanData> adaptationTask = VTask.delay(ForgeScanDataProvider::adaptLoaderData)
            .memoize()
            .unsafeRun();
    ...
    public void preload() {
        if (started.compareAndSet(false, true)) {
            Thread coordinator = ThreadConfig.platform("oelib-forge-scan-adapter-")
                    .factory()
        ...
        return Pathway.vtask(adaptationTask)
                .mapErrorAll(error -> {
                    if (error instanceof InterruptedException) {
                        return new IllegalStateException("Interrupted while adapting Forge scan data", error);
                    }
                    return new IllegalStateException("Failed to adapt Forge scan data", error);
                })
                .unsafeRun();
    }
    ...
    private static Object normalizeValue(Object value) {
        ...
        if (value instanceof ModAnnotation.EnumHolder holder) {
            return new OELScanData.EnumValue(holder.getDesc(), holder.getValue());
        }
        ...
        if (value instanceof Collection<?> collection) {
            return collection.stream().map(ForgeScanDataProvider::normalizeValue).toList();
        }
