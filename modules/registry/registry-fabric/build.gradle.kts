...
loom {
    val aw = project(":modules:registry:registry-common").file("src/main/resources/oelib-registry.accesswidener")
    if (aw.exists()) {
    ...
    runs {
        named("client") {
            client()
            configName = "Fabric Client - OELibRegistry"
            ideConfigGenerated(true)
            runDir("run")
        }
        named("server") {
            server()
            configName = "Fabric Server - OELibRegistry"
            ideConfigGenerated(true)
            runDir("run")
        }
    }
