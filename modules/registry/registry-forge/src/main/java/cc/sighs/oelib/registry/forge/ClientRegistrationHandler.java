...
public final class ClientRegistrationHandler {
    ...
    @Mod.EventBusSubscriber(modid = OELibRegistry.MOD_ID, value = Dist.CLIENT, bus = Mod.EventBusSubscriber.Bus.MOD)
    public static class ClientEventHooks {
        ...
        public static void onRegisterMenus(FMLClientSetupEvent event) {
            for (var ma : MENU_SCREENS) {
                registerMenuScreen(ma);
            }
        }

        private static <H extends AbstractContainerMenu, S extends Screen & MenuAccess<H>> void registerMenuScreen(MenuAction<H, S> action) {
            MenuScreens.register(action.type().get(), action.factory()::create);
        }
    }

    @Mod.EventBusSubscriber(modid = OELibRegistry.MOD_ID, value = Dist.CLIENT, bus = Mod.EventBusSubscriber.Bus.FORGE)
    public static class ForgeClientEventHooks {
        @SubscribeEvent
