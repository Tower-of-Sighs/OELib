...
public final class RegistrationDispatcherImpl implements IRegistrationDispatcher {
    private static final Map<ResourceKey<?>, DeferredRegister<?>> DR_CACHE = new ConcurrentHashMap<>();
    ...
    private <T> void performRegistryBatch(RegistryBatchAction<T> batch) {
        ...
        var dr = (DeferredRegister<T>) DR_CACHE.computeIfAbsent(key, k -> DeferredRegister.create(key, modid));
        dr.register(ModLoadingContext.get().getActiveContainer().getEventBus());
        for (RegisterSupplier<? extends T> entry : batch.entries()) {
            var ro = dr.register(entry.id().getPath(), () -> (T) entry.getCreator().get());
        ...
        public static void onClientSetup(FMLClientSetupEvent event) {
            event.enqueueWork(() -> {
                for (var rt : RENDER_BLOCKS) {
                    for (var b : rt.blocks()) ItemBlockRenderTypes.setRenderLayer(b, rt.type());
                }
                for (var rf : RENDER_FLUIDS) {
                    for (var f : rf.fluids()) ItemBlockRenderTypes.setRenderLayer(f, rf.type());
                }
            });
        }
    }
    ...
    public static final class ModEventHooks {
        ...
        public static void onFurnaceFuel(FurnaceFuelBurnTimeEvent event) {
            ...
            var time = FUEL_TIMES.getOrDefault(stack.getItem(), Integer.MIN_VALUE);
